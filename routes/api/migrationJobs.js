...
export function post(req, res, next) {
  ...
  let delete_source_after_migration =
    bodyData.delete_source_after_migration == true; // set to false if not set
  let keep_tags = bodyData.keep_tags == true; // set to false if not set
  ...
  if (!migrateFromSeries || !migrateToSeries || !chunkSize) {
    res.statusMessage =
      "Body data is not complete. Make sure fromSeries, toSeries and chunkSize are set";
    res
      .status(400)
      .send(
        "Body data is not complete. Make sure fromSeries, toSeries and chunkSize are set"
  ...
  Promise.all([
    m.seriesToChannel(migrateFromSeries),
    m.seriesToChannel(migrateToSeries),
  ]).then((channelObjects) => {
    let fromChannel = channelObjects[0];
    let toChannel = channelObjects[1];

    let jobDescription = m.createJobDescription(
      fromChannel,
      toChannel,
      chunkSize,
      src_influx,
      dest_influx,
      delete_source_after_migration,
      keep_tags
    );
    ...
    m.checkChannelExists(fromChannel, src_influx).then((exists) => {
      if (exists) {
        ...
        m.checkChannelExists(toChannel, dest_influx).then((exists) => {
          if (exists) {
            if (on_conflict == "drop") {
              ...
              console.log(
                new Date() +
                  "Data found in destination on conflict policy set to drop ! Dropping series !"
              );
              m.dropDestinationSeries(toChannel, dest_influx)
                .then(() => {
                  let guid = m.createMigrateTask(
                    fromChannel,
                    toChannel,
                    src_influx,
                    dest_influx,
                ...
                .catch((err) => {
                  ...
                  m.migrateTasks[jobDescription.guid].summary.status =
                    "Error dropping points !" + err;
              ...
              console.log(
                new Date() +
                  ` Data found in destination channel. Conflict policy set to merge. Merging series.`
              );
              let guid = m.createMigrateTask(
                fromChannel,
                toChannel,
              ...
              console.log(
                "There is already data in destination channel! Migration failed !"
              );
              m.migrateTasks[jobDescription.guid].summary.status =
                "There is already data in destination channel! Migration failed !";
              res
                .status(400)
                .send(
                  "There is already data in destination channel! Migration failed ! GUID : " +
                    jobDescription.guid
                );
              res.end();
            }
        ...
        m.migrateTasks[jobDescription.guid].summary.status =
          "There is no data in the source channel !";
        res
          .status(200)
          .send(
            "There is no data in the source channel ! GUID : " +
              jobDescription.guid
          );
        res.end();
      }
  ...
  */
  ...
  if (req.query.guid) {
    ...
    for (let i = objLength - 1; i >= 0; i--) {
      resObj[Object.keys(m.migrateTasks)[i]] = _.omit(
        Object.values(m.migrateTasks)[i],
        "chunks"
      );
    }
